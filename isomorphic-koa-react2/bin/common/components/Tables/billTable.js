...
var BillTable = function (_React$Component) {
  _inherits(BillTable, _React$Component);
  ...
  function BillTable(props) {
    ...
    generateTableBody = generateTableBody.bind(_this);
    return _this;
  }
...
function generateTableHeader(recordList) {
  var headerArray = recordList[0].row.slice(0);
...
function generateTableBody(recordList) {
  ...
  for (var i = 1; i < recordList.length; i++) {
    records.push(recordList[i].row.slice(0));
  }
  ...
  return _react2.default.createElement(
    ...
    records.map(function (elem, index) {
      return _react2.default.createElement(
        "tr",
        { key: 'btr_' + index, onClick: function onClick() {
            console.log(elem);
          } },
        elem.map(function (ele, indx) {
          return _react2.default.createElement(
